...
const availableFunctions = [
    {
        ...
        function: {
            ...
            description: "Create a reminder that will be sent to the WhatsApp group at a specific date and time. " +
            ...
            parameters: {
                ...
                properties: {
                    ...
                    datetime: {
                        type: "string",
                        description: "When to send the reminder. Supports natural language like: 'in 2 hours', 'tomorrow at 3pm', " +
                    ...
                    mentions: {
                        type: "array",
                        items: { type: "string" },
                        description: "Optional array of user phone numbers or names to mention in the reminder",
                    },
                },
    ...
    },
    {
        ...
        function: {
            name: "list_reminders",
            description: "List all reminders for this chat. Can filter by status (active, completed, or all).",
            parameters: {
                type: "object",
                properties: {
                    status: {
...
export class AiService {
     ...
     */
    async handleCreateReminder(args, chatId) {
        try {
            logger.info(`Creating reminder in chat ${chatId}:`, args);
            // Parse the datetime string
            const scheduledTime = reminderService.parseDateTime(args.datetime);
            // Create the reminder
            const reminder = await reminderService.createReminder(chatId, args.message, scheduledTime, args.mentions || []);
     ...
     */
    async handleListReminders(args, chatId) {
        try {
     ...
     */
    async handleCancelReminder(args, chatId) {
        try {
        ...
        try {
            ...
            if (!this.conversationHistory.has(userId)) {
                this.conversationHistory.set(userId, [
                    {
                        role: "system",
                        content: "Your name is Gigi. You are a helpful WhatsApp reminder bot assistant. " +
            ...
            // Extract final reply
            const reply = assistantMessage.content || "Sorry, I couldn't generate a reply.";
