...
class WhatsappService {
    ...
    async start(authPath = "./auth_info") {
        ...
        try {
            ...
            this.webSocket.ev.on("messages.update", (m) => logger.info(`Messages updated: ${JSON.stringify(m)}`));
            this.webSocket.ev.on("messages.upsert", (m) => logger.info(`Messages upserted: ${JSON.stringify(m)}`));
            ...
            this.webSocket.ev.on("creds.update", (creds) => {
                ...
                logger.info(`creds.me detected: ID=${creds.me.id}, LID=${creds.me.lid}, Name=${creds.me.name}, phone=${creds.me.phoneNumber}`);
    ...
    handleConnection(update) {
        ...
        if (connection === "open") {
            ...
            logger.info(`Bot LID: ${this.botLid}, JID: ${this.botJid}, Name: ${this.botName}`);
    ...
    async handleMessages({ messages }) {
        ...
        logger.info(`Incoming message from: ${senderId}, chat: ${chatId}, Extracted message text:'${text}'`);
    ...
    isBotMentioned(message) {
        ...
        logger.info(`Mentioned JIDs in this message: ${mentionedJids}`);
        const mentioned = mentionedJids.includes(this.botJid) ||
